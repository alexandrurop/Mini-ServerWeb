...
void send_insta_feed_to_socket(SOCKET client_socket) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        char* token = strtok(path_copy, "/"); // static
        token = strtok(NULL, "/");            // uploads
        token = strtok(NULL, "/");            // nume user
        if (token) strcpy(author, token);

        snprintf(buffer, sizeof(buffer),
            "<article class='post-container'>"
            "  <div class='post-header'>@%s</div>"   // autor sus
            "  <div class='post-image-wrapper'>"
            "    <img src='/%s' class='post-main-img' alt='Imagine'>"
            "  </div>"
            "  <div class='post-content-area'>"
            "    <p class='post-caption'>%s</p>"     // DOAR descrierea
            "    <div class='post-timestamp'>%s</div>"
            "  </div>"
            "</article>",
            author,
            all_posts[i].image_path,
            all_posts[i].description,   // descriere curată
...
void delete_post_local(const char* username, const char* image_path) {
    ...
    // 1. Curățăm calea primită (dacă începe cu / o scoatem pentru a se potrivi cu JSON-ul)
    ...
    if (target) {
        // Găsim începutul obiectului '{'
        char* start_obj = target;
        while (start_obj > buffer && *start_obj != '{') start_obj--;

        // Găsim sfârșitul obiectului '}'
        char* end_obj = strstr(target, "}");
        if (end_obj) {
            end_obj++; // Mergem după '}'

            // Gestionăm virgulele pentru a nu lăsa JSON-ul invalid
            if (start_obj > buffer && *(start_obj - 1) == ',') {
                start_obj--; // Ștergem virgula de dinainte
            }
            else if (*end_obj == ',') {
                end_obj++; // Sau ștergem virgula de după
            }

            // 3. Rescriem fișierul (MODUL "w" golește tot fișierul vechi)
