...
void start_server_engine(SOCKET server_socket, struct Route* route_tree) {
    ...
    for (int i = 0; i < POOL_SIZE; i++) {
        CreateThread(NULL, 0, worker_thread, route_tree, 0, NULL);
    }
...
DWORD WINAPI handle_client(LPVOID lpParam) {
    ...
    if (method != NULL && urlRoute != NULL) {
        ...
        if (strcmp(method, "POST") == 0 && strcmp(urlRoute, "/verify") == 0) {
            ...
            if (body) {
                if (sscanf(body, "username=%49[^&]&password=%49s", user, pass) == 2) {
                    login_ok = verify_user_in_db(user, pass);
                }
            }
            ...
            if (body && sscanf(body, "username=%[^&]&password=%s", user, pass) == 2) {
                ...
                if (rc == -1) {
                    ...
                    send_login_page_with_error(ctx->client_socket,
                        "Eroare server: nu pot crea contul.");
                }
                else {//succes
                    const char* success =
                        "HTTP/1.1 302 Found\r\n"
            ...
            char* response_data = render_static_file("static/login.html");
            ...
            if (template_h) {
                ...
                if (placeholder) {
                    ...
                    send(ctx->client_socket, template_h, front_len, 0);
        ...
        else if (strcmp(urlRoute, "/upload") == 0) {
            if (strcmp(method, "POST") != 0) {
                const char* red =
                    "HTTP/1.1 303 See Other\r\n"
                    "Location: /profile\r\n"
                    "Connection: close\r\n\r\n";
                send(ctx->client_socket, red, (int)strlen(red), 0);
                closesocket(ctx->client_socket);
                free(ctx);
                return 0;
            }

            // Body pointer (doar ce a intrat in primul recv)
            if (!body) {
            ...
            // Content-Length (dimensiunea BODY-ului)
            ...
            // Citim TOT body-ul intr-un buffer binar (NU string)
            ...
            // Copiem ce avem deja din primul recv (partea de body)
            ...
            // 1) EXTRAGEM DESCRIEREA (e text, deci folosim memmem pe buffer)
            ...
            // inceputul datelor de fisier = dupa "\r\n\r\n" in part-ul fisier
            ...
            // sfarsitul fisierului = inainte de "\r\n--<boundary>"
            char end_marker[256];
            sprintf(end_marker, "\r\n--%s", boundary_val);
            ...
            // 3) SALVARE PE DISC (asiguram folderul userului)
            ...
            char raw_path[256]="";
            if (body) {
                // Căutăm direct valoarea după "image_path="
                char* val = strstr(body, "image_path=");
                if (val) {
                    val += 11; // sărim peste "image_path="
                    // Copiem până la sfârșitul string-ului sau până la următorul parametru '&'
                    sscanf(val, "%255[^& \r\n]", raw_path);
                }
            }
            ...
            if (strlen(decoded_path) > 0) {
                // Folosim variabila LOCALĂ current_user (din cookie)
            ...
            if (file_data) {
                ...
                send(ctx->client_socket, header, (int)strlen(header), 0);
                send(ctx->client_socket, file_data, (int)actual_size, 0); // Trimitem dimensiunea EXACTĂ
